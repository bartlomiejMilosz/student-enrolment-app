...
public interface RentalService {
  ...
  /**
   * Rents a book to a student based on the provided book and student IDs. Ensures that the book is
   * available for rental and that the student is eligible to rent the book. Also, checks if the
   * student's ID card is active.
   *
   ...
   * @return a {@link RentalDto} object containing details of the rental transaction, including book
   *     and student identifiers, the rental date, and the due date
   ...
   */
  RentalDto rentBook(Long bookId, Long studentId, LocalDateTime dueDate);

  /**
   * Returns a book that was previously rented. This method handles the logic to mark a rented book
   * as returned, updating the inventory and the rental records.
   *
   ...
   */
  RentalDto returnBook(Long rentalId);
}
